...
class LearningPrior(Layer):
    def __init__(self, nb_gaussian, init=None, weights=None,
        ...
        if not init:
            self.init = gaussian_priors_init
            # self.init = initializers.RandomNormal(mean=0.5, stddev=0.1)
        else:
    ...
    def call(self, x):
        ...
        # print('LINSPACE SHAPE:',K.expand_dims(self._linspace(0, 1.0, self.width), axis=0).shape)
        # print('LINSPACE TYPE:',type(K.expand_dims(self._linspace(0, 1.0, self.width), axis=0)))
        ...
        # print('mu_x, mu_y, sig_x, sig_y',K.get_value(mu_x), K.get_value(mu_y), K.get_value(sigma_x), K.get_value(sigma_y))
    ...
    def _linspace(start, stop, num):
        ...
        # print('Type range after convert_to_tensor:', type(range))
        # print('variable "range" after convert_to_tensor:',range, range.shape)
        # print('range values":', K.get_value(range))
        #
        ...
        # print('step, type(step):',step, type(step))
        # # print('num value',K.get_value(num))
        #
        # range = K.arange(num, dtype='float32') * step + start
        # print('Type range with K.arange:',type(range), range)
        # print('variable "range" after K.arange:',range)
        # print('range values":', K.get_value(range))
        return range
